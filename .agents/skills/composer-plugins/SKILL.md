...
description: Use when working on files in packages/, adding new plugins,
...
A "plugin" is a package whose `src/meta.ts` exports a `Plugin.Meta`, so `ls packages/` overcounts (e.g. `plugin-generator` is tooling, not a plugin).
...
4. **At the end of Phase 1, before opening the PR**, author
   `packages/plugin-<name>/PLUGIN.mdl` from the design doc and the
...
Use the template as the starting structure and `packages/chess/PLUGIN.mdl`
...
See: `plugin-chess/src/components/Chessboard/`, `@dxos/react-ui-masonry/src/Masonry.tsx`
...
contract. See `packages/AUDIT.md` for the current registry.
...
```ts
// packages/foo/src/types/FooCapabilities.ts
...
A spec's `affinity` determines the slice it lives in and which fields of `LayerContext` are populated when its factory runs (see `@dxos/compute/src/LayerSpec.ts`):
...
- A failure for tag `X` will report `ServiceNotAvailable: X`, not the missing transitive dependency. If you need to debug WHY a spec was dropped, check the `pruned layer specs with unsatisfied requirements` log line emitted by `Slice.init` (`@dxos/compute-runtime/src/LayerStack.ts`).
...
A plugin's `moon.yml` declares no tasks of its own — it lists `tags`, and each tag pulls in the matching
