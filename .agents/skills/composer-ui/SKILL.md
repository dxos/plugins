...
truth is `@dxos/ui-theme/src/css/theme/semantic.css`
and the per-component files under `@dxos/ui-theme/src/css/components/`.
...
See `@dxos/react-ui/src/components/Icon/Icon.tsx`.

## Containers: Panel + ScrollArea

`Panel.*` (`@dxos/react-ui/src/components/Panel/Panel.tsx`)
...
1. **Need a picker / combobox** (choose from a set, typeahead)? **Check for an existing one first** —
   `Picker` / `Combobox` / `Listbox` in [`@dxos/react-ui-list`](../../../@dxos/react-ui-list/src/components),
...
`@dxos/react-ui-attention/AUDIT.md`):
...
See: `@dxos/react-ui-form/src/components/Form/Form.stories.tsx`
